...
class SentrySwitch:
    """
    Simulates a Zero-Trust SDN switch (The 'Sentry') that evaluates incoming traffic.
    
    The SentrySwitch orchestrates the simulation pipeline for each network stream,
    handling encryption simulation, topology traversal, feature extraction, 
    and model-based classification.
    """
    def __init__(self):
        """Initializes the switch with stats, logs, and loads the detection model."""
        self.stats = {"TP": 0, "TN": 0, "FP": 0, "FN": 0}
        self.simulation_logs = []
        self.model = self.load_model()
        
        # Mock Topology IP Addresses representing network nodes
        self.topo = {
            "Firewall": "10.0.0.1",
            "Core": "10.0.0.254",
            "Edge": "10.40.18.12"
    ...
    def load_model(self):
        """
        Loads the pre-trained Decision Tree model from the specified MODEL_PATH.
        
        Returns:
            sklearn.tree.DecisionTreeClassifier or None: The loaded model if successful.
        """
        try:
